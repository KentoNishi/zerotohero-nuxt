...
export default {
  methods: {
    ...
    async loadVideo(youtube_id, db_id) {
      this.video = { youtube_id };
      this.checkingSubs = true;
      try {
        const video = await this.getVideoFromDB(youtube_id, db_id);
    ...
    async getVideoFromDB(youtube_id, db_id) {
      let video;
      if (db_id) {
        video = await this.$directus.getVideo({ id: db_id, l2Code: this.$l2.code });
